...
$sql = 'SELECT m.*, p.nombre AS protectora_nombre, p.telefono AS telefono_protectora, p.email AS email_protectora, p.logo AS logo_protectora FROM mascotas m JOIN protectoras p ON m.idProtectora = p.idProtectora WHERE m.idMascota = ? AND m.activa = 1 LIMIT 1';
